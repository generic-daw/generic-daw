use crate::clap_host::PluginId;
use generic_daw_core::{
	PushSlot,
	clap_host::{self, HostInfo, MainThreadMessage, PluginDescriptor},
};
use std::sync::mpsc::Receiver;

#[derive(Debug)]
pub struct Plugin {
	pub id: PluginId,
	pub descriptor: PluginDescriptor,
	pub active: bool,
	pub mix: f32,
	pub s: oneshot::Sender<PushSlot<Option<generic_daw_core::Plugin>>>,
}

pub struct PluginPair {
	pub core: clap_host::Plugin,
	pub gui: Plugin,
}

impl PluginPair {
	pub fn new(
		descriptor: PluginDescriptor,
		host: HostInfo,
	) -> Option<(
		Self,
		PushSlot<Option<generic_daw_core::Plugin>>,
		Receiver<MainThreadMessage>,
	)> {
		let (core, receiver) = clap_host::Plugin::new(&descriptor, host)?;
		let (s, r) = oneshot::channel();
		let gui = Plugin {
			id: PluginId::unique(),
			descriptor,
			active: false,
			mix: 1.0,
			s,
		};
		Some((Self { core, gui }, PushSlot::new(Some(None), r), receiver))
	}
}
